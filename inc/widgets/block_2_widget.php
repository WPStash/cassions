 ...
 */
// Register the widget
add_action( 'widgets_init', create_function( '', 'return register_widget("Cassions_Widget_Block2");'));
// The widget class
class Cassions_Widget_Block2 extends WP_Widget {
	public function __construct() {
		$widget_ops = array('classname' => 'block2_widget', 'description' => esc_html__( "Display posts use on homepage.", 'cassions') );
		parent::__construct('cs_block2', esc_html__('WPS Block 2', 'cassions'), $widget_ops);
	...
	public function widget( $args, $instance ) {
		...
		$custom_query = new WP_Query( apply_filters( 'widget_block2_posts_args', $custom_query_args ) );
		...
		<div class="block2_widget_content">
			<?php while ( $custom_query->have_posts() ) : $custom_query->the_post(); $count++; ?>
                ...
                <article id="post-<?php the_ID(); ?>" <?php post_class( 'block-posts' ); ?>>
                    ...
                    <?php if ( has_post_thumbnail() ) { ?>
                    <div class="featured-image">
                        <?php if ( has_post_thumbnail() ) : ?><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_post_thumbnail('cassions-sidebar'); ?></a><?php endif; ?>
                    </div>
                    <?php } ?>
                    <!-- end .featured-image -->
