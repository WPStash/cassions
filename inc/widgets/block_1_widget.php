 ...
 */
// Register the widget
add_action( 'widgets_init', create_function( '', 'return register_widget("Cassions_Widget_Block1");'));
// The widget class
class Cassions_Widget_Block1 extends WP_Widget {
	public function __construct() {
		$widget_ops = array('classname' => 'block1_widget', 'description' => esc_html__( "Display posts use on homepage.", 'cassions') );
		parent::__construct('cs_block1', esc_html__('WPS Block 1', 'cassions'), $widget_ops);
	...
	public function widget( $args, $instance ) {
		...
		$number_posts        = ( ! empty( $instance['number_posts'] ) ) ? absint( $instance['number_posts'] ) : 13;
		if ( ! $number_posts ) $number_posts = 13;
		...
		<div class="block1_widget_content clear">
			<?php while ( $custom_query->have_posts() ) : $custom_query->the_post(); $count++; ?>
                ...
                <div class="block1-column-left">
                    ...
                    <!-- begin .hentry -->
                    <article id="post-<?php the_ID(); ?>" <?php post_class( 'block-posts first' ); ?> <?php echo $count; ?> >

                        <!-- begin .featured-image -->
                        <?php if ( has_post_thumbnail() ) { ?>
                        <div class="featured-image">
                            <?php if ( has_post_thumbnail() ) : ?><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_post_thumbnail( 'cassions-featured-post-large' ); ?></a><?php endif; ?>
                        </div>
                        <?php } ?>
                        <!-- end .featured-image -->
                ...
                ?>
                    ...
                    <article id="post-<?php the_ID(); ?>" <?php post_class( 'block-posts second' ); ?> >
                        ...
                        <?php if ( has_post_thumbnail() ) { ?>
                        <div class="featured-image">
                            <?php if ( has_post_thumbnail() ) : ?><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_post_thumbnail('cassions-featured-post'); ?></a><?php endif; ?>
                        </div>
                        <?php } ?>
                        <!-- end .featured-image -->
                        <!-- begin .entry-header -->
                ...
                <?php if ( $custom_query->found_posts == $count ): ?>
                </div> <!-- .block1-column-right -->
                <?php endif; ?>
